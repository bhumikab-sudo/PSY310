...
expName = 'Solo Lab Project Bhumika Bajaj AU2320057 PSY310'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\bhumika\\PSYLAB310\\Bhumika Bajaj Solo Lab Project PSY310 AU2320057\\Solo Lab Project Bhumika Bajaj AU2320057 PSY310_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text="Welcome \n\nIn this experiment, you will see words in different colors. Ignore the word itself. Press the key 'r' if the letter is red, 'b' if it is blue, 'p' if it is pink, and 'k' if it is yellow. Respond as quickly as you can. \n\nPress the 'space' key to start the experiment. ",
        font='Arial',
    ...
    inst = data.Routine(
        name='inst',
        components=[fixation, text, key_resp_2],
    )
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('emotionstrooptaskconditions.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            if target.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if target.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > target.tStartRefresh + 5.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['r', 'b', 'p', 'k'], ignoreKeys=["escape"], waitRelease=False)
