...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\bhumika\\PSYLAB310\\Adaptive Staircase Bhumika_AU2320057\\ADAPTIVESTAIRCASE_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisStair in stair:
        ...
        while continueRoutine:
            ...
            if grating.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > grating.tStartRefresh + 10.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['left','right'], ignoreKeys=None, waitRelease=False)
